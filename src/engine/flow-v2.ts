...
export async function routeStep({ sessionId, status, meta, userMessage }: RouteStepArgs): Promise<RouteStepResult> {
  ...
  if (session.phase === "intro") {
    ...
    return { status, meta: { step, phase: "intro" }, response: validation.response };
  }
  ...
  if (session.phase === "empathy") {
    ...
    return { status, meta: { step, phase: "empathy" }, response: validation.response };
  }
  ...
  if (session.phase === "deepening") {
    ...
    return { status, meta: { step, phase: "deepening" }, response: validation.response };
  }
